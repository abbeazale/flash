...
class FirebaseManager {
    ...
    func fetchRunningData() async -> [RunningData] {
        return await withCheckedContinuation { continuation in
            storage.collection("runs").getDocuments { snapshot, error in
                if let error = error {
                ...
                let runningDataArray = documents.compactMap { document -> RunningData? in
                ...
                continuation.resume(returning: runningDataArray)
            }
        }
    }
}
